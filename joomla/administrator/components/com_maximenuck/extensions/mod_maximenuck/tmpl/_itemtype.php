...
if ($item->deeper && $params->get('behavior', 'mouseover') === 'click' && $params->get('clicktoggler', '0') === '1') {
	$toggler = '<span class="maximenuck-toggler" tabindex="0" role="button" aria-label="Show submenu for ' . htmlspecialchars($item->ftitle, ENT_COMPAT, 'UTF-8', false) . '"></span>';
	$linktype .= $toggler;
} else {
	$toggler = '';
}
...
switch ($item->type) :
	default:
		...
		break;
	case 'separator':
		echo $opentag . '<span tabindex="0" role="button" ' . $linkrollover . ' ' . $datahover . ' class="separator ' . $item->anchor_css . '" data-align="' . $imagesalign . '">' . $linktype . '</span>' . $closetag;
		break;
	case 'heading':
		echo $opentag . '<span tabindex="0" role="button" ' . $linkrollover . ' ' . $datahover . ' class="nav-header ' . $item->anchor_css . '" data-align="' . $imagesalign . '">' . $linktype . '</span>' . $closetag;
		break;
	case 'url':
	case 'component':
