...
class CreativeContactFormViewCreativefields extends JViewLegacy {
    ...
    public function display($tpl = null) {
        ...
        $min_form_id  = $this->get('creativeMinFormId');
        ...
        if(JV == 'j2') {
            ...
            $filter_form_id = (int)$this->state->get('filter.form_id');
            $filter_form_id = $filter_form_id == 0 ? $min_form_id : $filter_form_id;
             
            JHtmlSidebar::addFilter(
                    JText::_('COM_CREATIVECONTACTFORM_SELECT_FORM'),
                    'filter_form_id',
                    JHtml::_('select.options', $options, 'value', 'text', $filter_form_id , true)
            );
