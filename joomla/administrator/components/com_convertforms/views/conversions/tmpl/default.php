                ...
                <?php foreach($this->items as $i => $item): ?>
                    ...
                    <tr class="row<?php echo $i % 2; ?> <?php echo isset($item->params->sync_error) ? 'error' : '' ?>">
                        ...
                        <td class="text-center">
                            ...
                            <?php } else { ?>
                                <div class="btn-group">
                                    <?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'conversions.', $canChange); ?>

                                    <?php
                                    if ($canChange && !defined('nrJ4'))
                                    {
                                        HTMLHelper::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'conversions');
                                        echo HTMLHelper::_('actionsdropdown.render', $this->escape($item->id));
                                    }
                                    ?>
                                </div>
                            <?php } ?>
